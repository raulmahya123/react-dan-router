...
import Carousel from '../../component/Breadcrumb/Carousel'

function Loading() {
  return Array(6).fill().map((_, index) => {
    return <div className='ps-4 relative card group' key={index}>
      <div
                    className="rounded-xl overflow-hidden  bg-gray-300 card-shadow relative"
                    style={{ width: 287, height: 386 }}
      ></div>
      <div className='w-25 h-3 bg-gray-300 mt-3 rounded-full'></div>
  ...
  return (
    <section className="flex flex-col py-16">
      ...
      <div className="container mx-auto" ref={refContainer}></div>
          ...
          {
          isLoading ? <div className='flex -mx-4 flex-row relative' style={{ paddingLeft: refContainer.current?.getBoundingClientRect?.(). left - 16 || 0 , }}
          >
            <Loading />
          </div>
             : error ? (
